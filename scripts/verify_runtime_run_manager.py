...
async def verify(event_store_path: Path, thread_prefix: str) -> dict[str, Any]:
    ...
    compat_manager = RunManager(event_store=store, runner=FakeRunner(store))  # type: ignore[arg-type]
    ...
    return {
        ...
        "run_manager_compat": isinstance(compat_manager, ExecutionEngine),
        "completed": {
...
def main() -> int:
    ...
    ok_compat = bool(summary["run_manager_compat"])
    ok_archived = bool(summary["archived"]["blocked"])
    passed = ok_completed and ok_cancelled and ok_approved and ok_rejected and ok_compat and ok_archived
    ...
    print(f"run_manager_compat={summary['run_manager_compat']}")
