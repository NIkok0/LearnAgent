...
### 2.3 Safety Gate 与 Approval 工作流
...
> **2026-05 进度**：P0–P4 核心能力已在 proxy 评测下验收；下表保留原始优先级划分，✅ 表示已落地。
...
- Approval
- Timeline
- 健康检查 / 任务查询 / 创建任务 demo（Demo 1 / 2 / 4 / 5 / 6）

### P1：任务排障能力 ✅

- QUEUED / PROCESSING / FAILED 诊断规则（`agent/diagnosis.py`）
- Worker / Redis / 对象存储排查建议
- 任务失败解释模板（post-tool SystemMessage 注入）

### P2：RAG 知识库 ✅

- 文档解析 + API 契约结构化 ingest
- chunk 元数据（含 `api_endpoint` / `request_fields` / `error_codes`）
- 关键词 + BM25 + 可选向量 + RRF
- 引用溯源（L4-lite + `retrieval_completed` sources）

### P3：Tool-grounded RAG ✅

- 问题分类（规则路由，`tool_router.py`）
- 检索驱动工具选择（`suggested_api_paths` / `api_field_hints`）
- 排障问答与实时状态结合（troubleshooting 优先于 live_status）

### P4：评估体系 ✅（proxy）

- Tool Calling eval（28 case L5 proxy）
- RAG eval（20 docs + 8 api/safety case）
