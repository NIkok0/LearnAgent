...
| Adapter | 职责 |
|---|---|
| `HttpResponseAdapter` | HTTP legacy dict → `ToolResultModel` |
...
### 2.4 检索依据事件（`retrieval_completed`）
...
`run_created`、`run_started`、`token`、`assistant_state`、`tool_start`、`tool_end`、`retrieval_completed`、`approval_required`、`approval_resolved`、`done`、`error`、`memory_run_summary`、`run_completed_meta` 等（完整列表见 `runtime/event_schema.py`）。
...
| 编排 | `agent/graph.py`、`agent/nodes.py` | LangGraph 图与安全闸门 |
...
`DomainEvent` 仍为 `RuntimeEvent` 的类型别名，便于旧代码渐进引用。
...
| 波次 | 层 | 任务 | 代码锚点 | 状态 | 验收 |
|------|-----|------|----------|------|------|
| **1** | L4 Pydantic | `memory_run_summary` / `memory_thread_summary` / `retrieval_completed` 专用 Pydantic 子模型 | `contracts/events/payloads.py` | ✅ | `verify_contract_events.py` |
| **1** | L4 | `GET /events?validated=1`（可选） | `server.py` | ✅ | `verify_events_validated.py` |
| **1** | L3→L4 | 统一 `ExtractedRecord`：RAG `api_parse` + Memory 抽取共用 validate | `contracts/extract.py` | ✅ | `verify_extract_validate.py` |
| **3** | L7 Output | `FinalAnswerModel`：NL + `citations[]` + 可选业务字段 envelope | `contracts/` + `event_mapper` | golden 断言扩展 |
| **3** | L7 | ToolMessage 摘要策略（全量 JSON vs 截断）文档化 + 配置项 | `tool_handlers.py` | eval case |
| **4** | L4 | MCP / 外部队列 tool 结果映射进 `ToolResultModel` | Adapter 层 | PoC verify |
