...
| 续轮去重 `SYSTEM_PROMPT` / 记忆 inject | ✅ | `memory/prompt_inject.py` |
...
| GDPR 删除 API / 真实鉴权 user 绑定 | ❌ Phase 3 | 见 §8 |
...
- `graph_input`：`SystemMessage(SYSTEM_PROMPT)` + episodic inject + `_to_lc_messages(turn_messages)`
...
`[EpisodicMemory]` 与 `[LongTermMemory]` 合并进单条 inject SystemMessage，位于 `SYSTEM_PROMPT` 之后、当前 user 之前（避免 Lost in the Middle）。续轮可通过 `memory_inject_dedupe_*` 跳过重复 System 消息。
...
回归入口：`scripts/verify_memory_checkpoint_consistency.py`，已纳入 `verify_eval_suite.py --profile core`（套件名 `memory_checkpoint_consistency`）。细节见 [eval-design.md](./eval-design.md)。
...
- **首轮 vs 续轮分支**：续轮不再向 `graph_input` 重复注入 `SYSTEM_PROMPT` 与 episodic `SystemMessage`，改为仅在 checkpoint 为空或策略版本变更时注入，避免 `add_messages` 累积重复 System 消息
...
路线图见 [agent-learning-guide.md](./agent-learning-guide.md) §7。

| 波次 | 层 | 任务 | 验收 |
|------|-----|------|------|
| **基线** | L2/L3/L5/L8 | Phase1 结构化 LTM；Phase2 向量/HyDE/LLM 抽取/pending；`checkpoint_compacted` 事件 | `verify_memory_production_v1/v2.py` |
| **1** | L3→L4 | Memory 抽取走统一 `ExtractedRecord`（`contracts/extract.py`） | ✅ `verify_extract_validate.py` |
| **2** | L5 Agent State | 续轮 inject 去重收尾（§8.1 与 `prompt_inject.py` 对齐） | `verify_memory_checkpoint_consistency.py` |
| **2** | L5 | 压缩时 hash 写入 episodic，防 checkpoint 丢细节 | 新 golden case |
| **2** | L2 | episodic run summary 可选向量索引（与 LTM 共用 embedder） | `verify_memory_production_v2.py` 扩展 |
| **3** | L8 | `export_checkpoint_messages.py` + 失败 run timeline 导出联动 | eval §7.4 |
...
| 问题 | 影响 | 说明 |
|---|---|---|
| 每轮仍注入 `SYSTEM_PROMPT` + episodic | checkpoint 内可能堆积多条内容相近的 `SystemMessage` | 与「checkpoint 为唯一历史」目标部分冲突；长线程依赖 Compactor 兜底 |
