...
| `HttpPathPolicy` + Scenario HTTP 白名单 | ✅ | `verify_scenario_loader.py`、`verify_rag_api_path_extraction.py` |
...
**验收**：`verify_scenario_loader.py`（allowlist 探针）、`verify_rag_api_path_extraction.py`（需 `apply_scenario_environment`）。
...
- 可选 `policy_decision` 调试事件（默认关闭以免噪声）。
