...
- 凡 **产品语义**（Run 状态、审批、取消、工具审计、memory 摘要）以 EventStore 为准。
...
| 主题 | 本文档 | 详见 |
|------|--------|------|
| 事件 kind、payload | §4 | [data-flow-design.md](./data-flow-design.md) |
| Run/timeline/审批 | §4 | [runtime-design.md](./runtime-design.md) |
| 工具参数脱敏（审计） | §5.3 边界 | [guardrail-policy-design.md](./guardrail-policy-design.md)、`tools/sanitize.py` |
| 选型四列（Trace 行） | — | [tech-selection-design.md](./tech-selection-design.md) §5 |
| Eval 回归 | — | EventStore/timeline 见 `verify_runtime_*`；Langfuse 无硬门禁 |
